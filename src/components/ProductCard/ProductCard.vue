...
function handleProductDetail() {
    store.dispatch('setProductDetail', props.product).then(() => {
        innerWidth < 512 ? router.push(`/mobile-product/${props.product.id}`) : router.push(`/product/${props.product.id}`)
...
<template>
    <div class="cursor-pointer w-80 flex items-center shadow-md rounded-xl overflow-hidden justify-between  bg-white flex-col p-2  hover:shadow-lg"
        ...
        <div class="my-5 flex items-center justify-between  w-full">
            <span v-if="product.price" class="font-bold text-lg text-gray w-1/3 flex justify-center">{{
    ...
    <div v-else
        class=" w-36 flex items-center shadow-md rounded-xl overflow-hidden justify-between  bg-white flex-col p-2 ">
        <div @click="handleProductDetail()" class="flex flex-col items-center justify-center cursor-pointer">
            <img v-lazy="product.image" class="object-scale-down h-24 w-24 " alt="">
            <div class=" h-12 self-start pt-2  cursor-pointer text-ellipsis overflow-hidden">
        ...
        <div class=" flex flex-col  items-center justify-center w-full  gap-2">
            ...
            <div class="flex flex-col h-8  w-full items-center justify-center">
                <span class="pl-2 cursor-pointer text-sm flex gap-1"
