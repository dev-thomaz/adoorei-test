...
const pixCode: string = '35375382309527304973058465983475092385923857623049672340967923847982347982374023580239582349582039570'
...
<template>
    <div class=" flex flex-col bg-white justify-center gap-8">
                ...
                <div class="bg-soft_white shadow-sm rounded-md flex flex-col p-2 gap-3">
                    ...
                    <div>
                        <div class="flex gap-3 items-center justify-between">
                            <span class="text-lg font-bold">{{ getTotalCount()  }} {{getTotalCount() > 1 ? 'Produtos:' : 'Produto'}} </span>
        ...
        <div class=" flex flex-col p-5 w-full">
            ...
            <div v-if="paymentSelected === 3" class=" p-2 flex flex-col items-center justify-center gap-3 h-80 mt-8">
                <span class="flex-wrap w-full">
                     Clique no código para copiar e cole no aplicativo do seu banco, na opção pix copia e cola
                </span>

                <div @click="handleCopypixCode()" class="h-10 w-80 border-2 border-soft_gray overflow-hidden flex justify-start items-center ">
                    <span class="w-10">
                        {{ pixCode }}
                    </span>
                </div>
                <div class="bg-positive flex justify-center items-center rounded-md p-3 w-1/2 text-white text-sm  opacity-0 transition-opacity duration-1000 " :class="{'opacity-95': showToast}" >Código pix copiado!</div>
    ...
    <div :class="{ hidden: !showModal }"
        class="fixed top-0 left-0 right-0 z-50 w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-modal h-screen bg-black_rgba flex justify-center items-center">
        <div class="bg-soft_white w-full h-1/3 rounded-lg p-5 flex flex-col  items-center justify-center gap-3">
            <span class=" text-lg font-bold text-gray">Pedido realizado com sucesso</span>
            <span>número do pedido 0212-5984/2023</span>
            <span>confirmação do pedido enviada para: </span>
            <span class="text-lg font-bold">{{userLogged.email}}</span> 
